 ...
 *
 *  ByteBUffer.allocateDirect(capability) 第一种凡是是分配OS本地内存，不属于GC管辖范围，由于不需要内存拷贝所以速度相对较快。
 *
 *  但如果不断分配本地内存，堆内存很少使用，那么JVM就不需要执行GC，DirectByteBuffer对象们就不会被回收。
 *  这时候堆内存充足，但本地内存可能已经使用光了，再次舱室分配本地内存就会出现OutOfMemoryError，程序就直接崩溃了。
 *
 **/
